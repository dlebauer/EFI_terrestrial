...
sensor1$weight<-weighted_sensors$weight[match(sensor1$spd, weighted_sensors$spd)]
...
wt_avg_soil_moisture<-aggregate(list(moisture=sensor1$weighted_moisture), by=
                       list(siteID=sensor1$siteID, startDateTime=sensor1$startDateTime, day=sensor1$day, spd=sensor1$spd), FUN="sum", na.rm=T)
...
sensor_moisture_uncert<-aggregate(list(sq_uncert=sensor1$sq_weighted_std_dev), by=
                       list(siteID=sensor1$siteID, startDateTime=sensor1$startDateTime, day=sensor1$day, spd=sensor1$spd), FUN="mean", na.rm=T)
...
wt_avg_soil_moisture$std_dev_uncert<-sensor_moisture_uncert$std_dev_uncert[match(wt_avg_soil_moisture$spd,sensor_moisture_uncert$spd)]
...
target_daily_moisture<-aggregate(
  list(moisture=target_30m_moisture$moisture , uncertainty=target_30m_moisture$std_dev_uncert), 
  by=list(siteID=sensor1$siteID, day=sensor1$day), FUN="mean", na.rm=T)
