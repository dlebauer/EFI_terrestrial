...
ggplot(efi, aes(x=DOY, y=data.fluxCo2.nsae.flux, col=siteID))+geom_point()+theme_bw()+facet_grid(siteID~ Year, scales="free_x")+
...
ggplot(t, aes(x=DOY, y=x, col=siteID))+geom_point()+theme_bw()+facet_grid(siteID~ Year, scales="free_x")+
...
ggplot(t, aes(x=x, fill=siteID))+geom_histogram()+theme_bw()+facet_grid(siteID~ Year, scales="free_x")+
  ylab("Number of days")+xlab("Number of half hour observations per day")
